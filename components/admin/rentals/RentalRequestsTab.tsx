    ...
    try {
      ...
      if (res.ok) {
        setRequests((prev) =>
          prev.map((r) =>
            r.id === id
              ? { ...r, status: status as RentalRequest["status"] }
              : r
          )
        );
      }
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-border/30 shadow-sm text-center py-16">
            ...
            <p className="text-lg font-semibold text-primary mb-1">Aucune demande</p>
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 flex-wrap mb-1">
                      <h3 className="text-sm font-bold text-primary truncate">{req.productName}</h3>
                      {req.variant && (
                        <span className="px-2 py-0.5 bg-primary/5 text-xs font-medium text-primary rounded-md">{req.variant}</span>
                      )}
                  ...
                  <ChevronDown className={`w-5 h-5 text-muted-foreground transition-transform duration-300 flex-shrink-0 ${isExpanded ? "rotate-180" : ""}`} />
                </button>
                ...
                {isExpanded && (
                  <div className="px-6 pb-6 border-t border-border/30 pt-4 animate-fade-in">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-semibold text-primary">{req.customerName}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <a href={`mailto:${req.customerEmail}`} className="text-sm font-semibold text-primary hover:text-accent transition-colors">{req.customerEmail}</a>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <a href={`tel:${req.customerPhone}`} className="text-sm font-semibold text-primary hover:text-accent transition-colors">{req.customerPhone}</a>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-semibold text-primary">
                            {req.duration} {req.durationUnit === "jour" ? "jour(s)" : req.durationUnit === "heure" ? "heure(s)" : "an(s)"}
                          </p>
                        </div>
                      </div>
                      {req.variant && (
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-semibold text-primary">{req.variant}</p>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    {req.remarks && (
                      <div className="mb-5 p-4 bg-neutral/30 rounded-xl">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <p className="text-xs font-semibold text-muted-foreground">Remarques</p>
                        </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <p className="text-xs font-semibold text-muted-foreground self-center mr-2">Changer le statut :</p>
