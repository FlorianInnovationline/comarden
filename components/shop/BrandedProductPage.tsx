...
/**
 * Rich, brand-themed product page driven entirely by the DB product
 * (title, description, specs, avantages, variants, images). Colours + name come
 * from the brand config, so it works for any brand that has a /marques page.
 */
export default function BrandedProductPage({
  ...
  return (
    <div className="pt-20 bg-white">
      ...
      <section className="py-4 bg-neutral/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex flex-wrap items-center gap-2 text-sm">
            ...
            <span className="text-muted-foreground">/</span>
            <Link href={`/marques/${brand.slug}`} className="text-muted-foreground hover:text-primary transition-colors">{brand.name}</Link>
            <span className="text-muted-foreground">/</span>
      ...
      <section className="py-8 sm:py-12 lg:py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
            <Reveal>
              <ProductGallery images={product.images} alt={product.title} accent={accent} />
            </Reveal>
            <Reveal delay={100}>
              <div>
                <span
      ...
      {specs.length > 0 && (
        <section className="py-12 sm:py-16" style={{ backgroundColor: `${primary}0D` }}>
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <Reveal>
              <div className="flex items-center gap-3 mb-8">
                <span className="h-7 w-1.5 rounded-full" style={{ backgroundColor: accent }} />
              ...
              {variants.map((v, i) => (
                <Reveal key={v} delay={i * 30}>
                  <div className="flex items-start gap-3 rounded-xl ring-1 ring-black/5 bg-slate-50 px-4 py-3">
                    <Package className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: accent }} />
      ...
      <section className="py-14 sm:py-20" style={{ backgroundColor: primary }}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
          <Reveal>
            <p className="text-xs sm:text-sm font-bold uppercase tracking-[0.2em] mb-4" style={{ color: accent }}>
            ...
            <p className="text-base sm:text-lg text-white/80 max-w-2xl mx-auto mb-8 leading-relaxed">
              Notre équipe vous accompagne dans le choix et la mise en œuvre de vos solutions {brand.name},
