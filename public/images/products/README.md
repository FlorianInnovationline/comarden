...
- **Côté boutique** : les images affichées = **fusion** du champ `images` (seed, admin) **+** les fichiers image présents dans ce dossier (`jpg`, `jpeg`, `png`, `webp`, `gif`). Ordre : URLs enregistrées d’abord, puis fichiers du dossier (sans doublon). Les fichiers dont le nom commence par `main` sont listés en premier.
